...
export default class Auth extends PageManager {
    ...
    loaded(next) {
        ...
        $('.account-create-form').find('#FormField_1').after('<div class="clearfix"></div>');
        $('.account-create-form').find('#FormField_3').nextAll().wrapAll('<div class="vip_member_container"></div>');
        
        // Appending the VIP form , from 'Login Details' to 'Delivery Details' section
        $('.account-create-form').find('.account-create-address').append($('.vip_member_container').html());
        $('.account-create-form').find('.vip_member_container').remove();

        // Customizing text of VIP member confirmation label and making a particular radio button checked always by default
        $('.account-create-form').find('.account-create-address').find('#FormField_25').prepend("<p class='required-note'><em>*Required field</em></p><h5>Kidstuff Rewards</h5><p class='rewarded'>Be rewarded for every dollar you spend with Kidstuff both online and in-store! <br> Would you like to sign up for Kidstuff Rewards?</p>");

        document.getElementById('FormField[1][25]_0').checked = true;

        $('.account-create-form input').on('change', function() {
            var radioVal= $(this).val();
            if(radioVal == 'Yes, I would like to sign up'){
                //show
                $('.reward').show();
                $('.form-field--checkbox').show();

            }else if(radioVal=='No, not yet'){
                //hide
                $('.reward').hide();
        ...
        $('.account-create-form').find('.account-create-address').find('.form-field:nth-of-type(2n)').after('<span class="clearfix"></span>');

        $('.account-create-form').find('.account-create-address').find('#FormField_25 label:first').hide();
        $('.account-create-form').find('.account-create-address').find('#FormField_26 label:first').hide();
        $('.account-create-form').find('.account-create-address').find('#FormField_27 label:first').hide();
        $('.account-create-form').find('.account-create-address').find('#FormField_25').append("<p class='reward'>By signing up to Kidstuff Rewards you are <br> agreeing to the <a href='\/rewards-terms'>terms & conditions<\/a> of this loyalty program</p>");
